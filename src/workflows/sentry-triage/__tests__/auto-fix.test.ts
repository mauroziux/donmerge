...
// Mock fetch globally — no sandbox/FlueRuntime mocks needed;
// flue is now provided via context.flue in tests.
...
describe('runAutoFix', () => {
  ...
  it('should return null when patched_content is identical to current', async () => {
    ...
    // Context has matching sourceCode key for the triage output's affected_files
    const context = makeContext({
  ...
  it('should produce correct branch name format', async () => {
    ...
    const context = makeContext({ sentryIssueId: '99887' });
    ...
    expect(body.ref).toBe('refs/heads/donmerge/fix/sentry-99887-aaaaaaaa');
  });
  ...
  it('should sanitize PR title', async () => {
    ...
    expect(body.title).toContain('fix(sentry):');
  });
  ...
  it('should include expected sections in PR body', async () => {
    ...
    expect(prBody).toContain('DonMerge Sentry Triage');
    expect(prBody).toContain('Sentry Issue');
    ...
    expect(prBody).toContain(context.sentryIssueUrl);
  });
